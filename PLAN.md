...
- **Later** — ✅ recall-by-brand-size measurement (`lib/matcher/recall.ts`, `npm run db:recall`); ✅ logistic-regression matcher trained on labeled pairs (`lib/matcher/logistic.ts` + `labeling.ts` + `eval-pairs.ts`, `npm run matcher:train`; the learned model in `model.ts` is now the live `score.ts` scorer, threshold re-tuned to 0.887); ✅ head-brand sibling disambiguation (`variantConflict` feature in `features.ts` for SPF/shade/concentration/AM-PM + scent/color/pack-count, variant-conflict negatives in `labeling.ts`; hand-labeled eval 95%→100% with the sibling cases fixed; `db:recall` sampling now seeded/reproducible — note: full-catalog head recall is bottlenecked by generic-name *containment*, "Shampoo"/"Hand Soap" rows, not variant siblings, which is the real next precision lever); ⬜ generic-name containment fix; ⬜ MinHash blocking; ⬜ a second product vertical.
