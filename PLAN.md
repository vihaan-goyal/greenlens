...
- **Later** — ✅ recall-by-brand-size measurement (`lib/matcher/recall.ts`, `npm run db:recall`); ✅ logistic-regression matcher trained on labeled pairs (`lib/matcher/logistic.ts` + `labeling.ts` + `eval-pairs.ts`, `npm run matcher:train`; the learned model in `model.ts` is now the live `score.ts` scorer, threshold re-tuned to 0.8845); ⬜ MinHash blocking; ⬜ a second product vertical.
